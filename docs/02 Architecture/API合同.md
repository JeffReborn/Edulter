...
### 持久化语义（与实现对齐）
- 生成所依据的为该客户 **当前有效画像**（`isCurrent`）；成功写入前，会在**同一数据库事务**内删除该 `clientId` + 该画像 `profileId` 下已存在的所有 `generated_followups` 行，再插入本次请求的各 `styleTypes` 记录。
- 因此：**同一客户、同一当前画像**下重复调用本接口为**整批替换**，而非无限追加；画像更新导致 `profileId` 变化后，旧画像下已保存的草稿行仍保留在库中（除非另行清理）。
...
默认仅聚合 **当前有效画像**（`latestProfile`）下已保存的跟进草稿（至多若干条，按时间倒序），与「同画像下再次生成会替换旧草稿」的持久化策略一致。
